# 山行判断ツール V5.5.1 — 日本アルプス・プリロード登山道版


## V5.5.1 修正

V5.5で内蔵登山道ルートが正常に見つかった際、内蔵ルート件数カウンター `preloadedHits` が未初期化のため `ReferenceError` になり、「登山道生成エラー」と表示される不具合を修正しました。内蔵登山道・区間キャッシュ・Overpassフォールバックの探索順序は変更していません。
...
## V5.5.1 日本アルプス・プリロード登山道版

V5.5.1では、ルート探索時に毎回Overpass APIから登山道ネットワークを取得する方式を改め、Renderのデプロイ時に日本アルプス主要山域のOpenStreetMap登山道データを取得して、圧縮済みルート探索グラフとしてアプリ内へ生成します。
...
1. ブラウザの30日区間キャッシュ
2. V5.5.1の内蔵OSM登山道グラフ（`/api/trail-route`）
3. Overpass APIの狭域検索
4. Overpass APIの広域検索
...
`/api/health` の `trail_regions_ready` で、内蔵データが正常生成できた山域数を確認できます。V5.5.1では通常 `8` が目安です。
